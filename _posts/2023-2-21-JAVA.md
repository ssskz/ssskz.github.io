...
It can be seen that the efficiency of `clone()` and `arraycopy()` is basically the same, while the efficiency of copying through loops is the slowest.
...
Simply put, a Native Method is an interface that Java calls non-Java code. A Native Method is a Java method whose implementation is implemented by a non-Java language, such as C. This feature is not unique to Java, but is found in many other programming languages, such as in C++, where you can use extern "C" to tell the C++ compiler to call a C function.
------
