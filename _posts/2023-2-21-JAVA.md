...
In the above code, since the length of the original array scores is 5, and the length of the new array newScores to be copied is 8, after the 5 elements in the original array are copied, the contents of the remaining 3 elements are filled with default values.
